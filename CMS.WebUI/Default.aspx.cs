...
using CMS.WebUI.Administration;
...
namespace CMS.WebUI
{
    public partial class _Default : Page
    {
        ...
        private void BindGioiThieu()
        {
            ...
            if (DefaultSettingBLL.GetByValueString(KeyName.chkGioiThieuHienThiHome) == StatusSetting.ValueTrue)
            {
                stringBuilder.AppendFormat(templateAbout.InnerHtml,
                    DefaultSettingBLL.GetByValueString(KeyName.txtGioiThieuTieuDeHome),
                    Server.HtmlDecode(DefaultSettingBLL.GetByValueString(KeyName.txtGioiThieuContentHome)),
                    DefaultSettingBLL.GetByValueString(KeyName.txtImageHome));
            }
            ltrGioiThieu.Text = stringBuilder.ToString();
        }

        private void BindKhamPha()
        {
            StringBuilder stringBuilder = new StringBuilder();
            if (DefaultSettingBLL.GetByValueString(KeyName.chkKhamPhaHienThiHome) == StatusSetting.ValueTrue)
            {
                stringBuilder.AppendFormat(templateKhamPha.InnerHtml,
                    DefaultSettingBLL.GetByValueString(KeyName.txtKhamPhaTieuDeHome),
                    DefaultSettingBLL.GetByValueString(KeyName.txtKhamPhaNoiDungHome),
                    DefaultSettingBLL.GetByValueString(KeyName.txtKhamPhaTamNhinHome),
                    DefaultSettingBLL.GetByValueString(KeyName.txtKhamPhaSuMenhHome),
                    DefaultSettingBLL.GetByValueString(KeyName.txtKhamPhaGiaTriCotLoiHome),
                    DefaultSettingBLL.GetByValueString(KeyName.txtHinhNen));
            }
            ltrKhamPha.Text = stringBuilder.ToString();
        }
        private void BindNganhDaoTao()
        {
            StringBuilder stringBuilder = new StringBuilder();
            if (DefaultSettingBLL.GetByValueString(KeyName.chkNganhDaoTaoHienThiHome) == StatusSetting.ValueTrue)
            {
                DanhMuc objNhomNganh = DanhMucBaiVietBLL.GetById(DefaultSettingBLL.GetByValueString(KeyName.ddlNganhDaoTaoNhomHone));
                if (objNhomNganh != null)
                {
                    stringBuilder.AppendFormat(templateNganhDaoTao.InnerHtml,
                        objNhomNganh.Ten,
                        BindItemNganhDaoTao(objNhomNganh.Id));
                }

            }
            ltlrNganhDaoTao.Text = stringBuilder.ToString();
        }

        private string BindItemNganhDaoTao(int idDanhMuc)
        {
            List<NganhDaoTao> list = NganhDaoTaoBLL.GetTopListNganhByDanhMucId(idDanhMuc, DefaultSettingBLL.GetByValueString(KeyName.txtNganhDaoTaoSoLuongHome));

            StringBuilder stringBuilder = new StringBuilder();
            foreach (var item in list)
            {

                stringBuilder.AppendFormat(templateItemNganhDaoTao.InnerHtml,
                    item.TenNganh, Helpers.GetThumbnailUrl(item.ThumbnailUrl),
                    item.Slug
                   );
            }
        ...
        private void BindQuyMo()
        {
            StringBuilder stringBuilder = new StringBuilder();
            if (DefaultSettingBLL.GetByValueString(KeyName.chkQuyMoHienThiHome) == StatusSetting.ValueTrue)
            {
                DanhMuc objNhomNganh = DanhMucBaiVietBLL.GetById(DefaultSettingBLL.GetByValueString(KeyName.ddlNganhDaoTaoNhomHone));
                if (objNhomNganh != null)
                {
                    stringBuilder.AppendFormat(templateQuyMo.InnerHtml,
                        DefaultSettingBLL.GetByValueString(KeyName.txtSoLuongTieuDe1Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMoSoLuongHome),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMoTieuDe2Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMosoLuong2Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMoTieuDe3Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMosoLuong3Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMoTieuDe4Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMosoLuong4Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMoTieuDe5Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtQuyMosoLuong5Home),
                        DefaultSettingBLL.GetByValueString(KeyName.txtHinhNen));
                }

            }
            ltrQuyMo.Text = stringBuilder.ToString();
        }

        private void BindNganhSuKien()
        {
            StringBuilder stringBuilder = new StringBuilder();
            if (DefaultSettingBLL.GetByValueString(KeyName.chkSuKienHienThi) == StatusSetting.ValueTrue)
            {
                DanhMuc objDoiTac = DanhMucBaiVietBLL.GetById(DefaultSettingBLL.GetByValueString(KeyName.ddlSuKienNhomHone));
                if (objDoiTac != null)
                {
                    stringBuilder.AppendFormat(templateSuKien.InnerHtml,
                        objDoiTac.Ten,
                        BindItemSuKien(objDoiTac.Id));
                }

            }
            ltrSuKien.Text = stringBuilder.ToString();
        }

        private string BindItemSuKien(int idDanhMuc)
        {
            List<BaiViet> list = BaiVietBLL.GetTopListBaiViet(idDanhMuc, DefaultSettingBLL.GetByValueString(KeyName.txtSuKienSoLuongHome));

            StringBuilder stringBuilder = new StringBuilder();
            foreach (var item in list)
            {

                stringBuilder.AppendFormat(templateItemSuKien.InnerHtml,
                    item.TieuDe,
                    Helpers.GetThumbnailUrl(item.ThumbnailUrl),
                    item.Slug,
                    Server.HtmlDecode(item.MoTaNgan)
                   );
            }
            return stringBuilder.ToString();
        }
        ...
        private void BindNganhTinTuc()
        {
            StringBuilder stringBuilder = new StringBuilder();
            if (DefaultSettingBLL.GetByValueString(KeyName.chkTinTucHienThi) == StatusSetting.ValueTrue)
            {
                DanhMuc objDoiTac = DanhMucBaiVietBLL.GetById(DefaultSettingBLL.GetByValueString(KeyName.ddlTinTucNhomHone));
                if (objDoiTac != null)
                {
                    stringBuilder.AppendFormat(templateTinTuc.InnerHtml,
                        objDoiTac.Ten,
                        BindItemTinTuc(objDoiTac.Id));
                }

            }
            ltrTinTuc.Text = stringBuilder.ToString();
        }

        private string BindItemTinTuc(int idDanhMuc)
        {
            List<BaiViet> list = BaiVietBLL.GetTopListBaiViet(idDanhMuc, DefaultSettingBLL.GetByValueString(KeyName.txtTinTucSoLuongHome));

            StringBuilder stringBuilder = new StringBuilder();
            foreach (var item in list)
            {

                stringBuilder.AppendFormat(templateItemTinTuc.InnerHtml,
                    item.TieuDe,
                    Helpers.GetThumbnailUrl(item.ThumbnailUrl),
                    item.Slug,
                    Server.HtmlDecode(item.MoTaNgan),
                    item.NgayTao.ToString("dd"),
                    item.NgayTao.ToString("MM/yyyy")
                   );
            }
            return stringBuilder.ToString();
        }
        private void BindNganhThanhVien()
        {
            StringBuilder stringBuilder = new StringBuilder();
            if (DefaultSettingBLL.GetByValueString(KeyName.chkThanhVienHienThi) == StatusSetting.ValueTrue)
            {
                DanhMuc objDoiTac = DanhMucBaiVietBLL.GetById(DefaultSettingBLL.GetByValueString(KeyName.ddlThanhVienNhomHone));
                if (objDoiTac != null)
                {
                    stringBuilder.AppendFormat(templateThanhVien.InnerHtml,
                        objDoiTac.Ten,
                        BindItemThanhVien(objDoiTac.Id));
                }

            }
            ltrThanhVien.Text = stringBuilder.ToString();
        }

        private string BindItemThanhVien(int idDanhMuc)
        {
            List<BaiViet> list = BaiVietBLL.GetTopListBaiViet(idDanhMuc, DefaultSettingBLL.GetByValueString(KeyName.txtThanhVienSoLuongHome));

            StringBuilder stringBuilder = new StringBuilder();
            foreach (var item in list)
            {

                stringBuilder.AppendFormat(templateItemThanhVien.InnerHtml,
                    item.TieuDe, 
                    Helpers.GetThumbnailUrl(item.ThumbnailUrl),
                    item.Slug
                   );
        ...
        {
            if (DefaultSettingBLL.GetByValueString(KeyName.chkDoiTacHienThi) == StatusSetting.ValueTrue)
            {
                List<DoiTac> doiTacs = DoiTacBLL.GetTopListDoiTac(DefaultSettingBLL.GetByValueString(KeyName.txtDoiTacTieuDeHome));
                string a = "";
